...
import { Product, PurchaseOrder, PurchaseOrderItem, StaffMember, Supplier } from '../../types';
...
interface Props { vendorId: string; businessName: string; currency: string; products: Product[]; activeStaff: StaffMember; onChanged: () => void; }
type DraftLine = Omit<PurchaseOrderItem, 'receivedQuantity'>;

export function PurchaseOrderWorkspace({ vendorId, businessName, currency, products, activeStaff, onChanged }: Props) {
  ...
  const useRecommendations = () => { setSource('BI_RECOMMENDATION'); setLines(recommendations.map(({ systemQuantity: _system, recommendedQuantity: _recommended, rationale: _rationale, ...line }) => line)); };
  ...
  return <div className="space-y-6">
    <header className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="flex items-center gap-2 text-xl font-black"><ShoppingCart className="text-[#FF6600]"/>Purchase Orders</h1><p className="text-xs text-slate-500">Planning and approval only. Inventory changes exclusively through approved stock receiving.</p></div><button onClick={useRecommendations} disabled={!recommendations.length} className="flex items-center gap-2 rounded bg-[#1F242D] px-4 py-2 text-xs font-bold text-white disabled:opacity-40"><BrainCircuit className="h-4 w-4"/>Use {recommendations.length} BI recommendations</button></header>
    {error && <p className="rounded border border-red-200 bg-red-50 p-3 text-xs font-bold text-red-700">{error}</p>}
    <section className="rounded-xl border bg-white p-4 shadow-sm space-y-4">
      <div className="grid gap-3 md:grid-cols-3"><label className="text-xs font-bold">Supplier<select value={supplierId} onChange={e=>setSupplierId(e.target.value)} className="mt-1 w-full rounded border p-2"><option value="">Select supplier</option>{suppliers.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></label><label className="text-xs font-bold">PO source<select value={source} onChange={e=>setSource(e.target.value as typeof source)} className="mt-1 w-full rounded border p-2"><option value="PLANNED">Planned purchase</option><option value="BI_RECOMMENDATION">BI recommendation</option></select></label><label className="text-xs font-bold">Notes<input value={notes} onChange={e=>setNotes(e.target.value)} className="mt-1 w-full rounded border p-2"/></label></div>
