...
| PUR-001 | Purchase order creation, approval and issue create no stock | PURCHASING | `functions/src/purchaseOrderLifecycle.ts`; trusted callable commands never invoke inventory posting | `purchaseOrderLifecycle.test.ts` | Callable emulator coverage required | Direct PO writes denied | Functions/root build | Partial |
| PUR-002 | PO lifecycle transitions are explicit and state-dependent | PURCHASING | `transitionPurchaseOrder`; callable create/submit/approve/reject/cancel/issue/close commands | State, cancellation, closure and stale-decision tests | Callable emulator coverage required | Direct status mutation denied | Required | Partial |
| PUR-003 | PO authority is tenant, permission, role and warehouse scoped | PURCHASING | Transactional membership/role resolution in `purchaseOrderCommands.ts` | Unauthenticated, non-member, wrong-tenant, permission and warehouse tests | Callable emulator coverage required | `tests/firestore.rules.test.ts` | Required | Partial |
| PUR-004 | Server owns product snapshots and commercial totals | PURCHASING | `calculateCommercialLines`; canonical product reads in callable transaction | Total, invalid product, negative and non-finite tests | Callable emulator coverage required | Direct total mutation denied | Required | Partial |
| PUR-005 | Approval enforces version, thresholds and segregation of duties | PURCHASING; APPROVAL_WORKFLOWS | Versioned approval request and `assertApprovalAllowed` | Unauthorized, threshold, stale and self-approval tests | Callable emulator coverage required | Direct PO approval mutation denied | Required | Partial |
| PUR-006 | Issued commercial history is immutable | PURCHASING | Draft-only `amendPurchaseOrder`; approved commercial snapshot | Draft edit and issued immutability tests | Callable emulator coverage required | Direct PO update denied | Required | Partial |
| PUR-007 | PO receiving consumes server-owned outstanding quantities | PURCHASING; SUPPLIER_RECEIVING | `postSupplierReceipt`; `applyPurchaseOrderReceipt`; one Firestore transaction owns receipt, buckets, ledger, PO and events | Lifecycle and accounting tests | `tests/supplierReceiving.callable.test.ts` (35 cases) | Auth + Functions + Firestore callable gate | Production build | Verified |
| PUR-008 | Procurement commands and events are idempotent and server-owned | PURCHASING; BI_EVENT_LOGGING | Server-only command registry and deterministic audit/BI event IDs | Replay and command-identity conflict tests | Callable emulator coverage required | Command/event writes denied | Required | Partial |
| PUR-009 | Received POs cannot be silently deleted and closure is controlled | PURCHASING | `RECEIVED -> CLOSED` only; all PO deletes denied | Closure and cancellation tests | Required | `tests/firestore.rules.test.ts` | Required | Partial |
| REC-001 | Accepted receipt posts warehouse stock once | SUPPLIER_RECEIVING | Canonical `InventoryPostingEngine` invoked by `postSupplierReceipt`; rejected quantity remains history-only | Canonical engine and quantity reconciliation tests | `tests/supplierReceiving.callable.test.ts` (35 cases) | Atomic rollback, replay, concurrency, disposition and rules coverage | Production build | Verified |
| REC-002 | Receipt reversal is compensating, idempotent and restores PO accounting | SUPPLIER_RECEIVING; INVENTORY_LEDGER | `reverseSupplierReceipt`; `SUPPLIER_RECEIPT_REVERSAL`; `reversePurchaseOrderReceipt` | Canonical reversal bucket behavior | `tests/supplierReceiving.callable.test.ts` | Original immutability, insufficient stock, replay and rollback coverage | Production build | Verified |
...
| STK-001 | Blind count hides expected quantity | STOCKTAKE_AND_ADJUSTMENTS | `submitStocktakeCount` stores server snapshot but redacts it from blind response | Lifecycle tests | `tests/stocktake.callable.test.ts` (30 cases) | Rules deny evidence writes | Production build | Verified |
...
| STK-003 | Approved adjustment posts once and reverses only by compensation | STOCKTAKE_AND_ADJUSTMENTS | Canonical engine posting and partial/full reversal with snapshot/version guard and command registry | `inventoryCore.test.ts` | `tests/stocktake.callable.test.ts` (30 cases) | Direct lifecycle, evidence, command, movement, balance and event writes denied | Production build | Verified |
